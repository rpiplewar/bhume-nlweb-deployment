...
# Check if data was loaded at build time
echo "=== Checking Data Status ==="
if [ -d "/app/data/db" ] && [ "$(ls -A /app/data/db)" ]; then
    echo "Data directory exists and contains files - data was loaded at build time"
    DATA_LOADING_SUCCESS=true
else
    echo "No data found - attempting runtime data loading as fallback"
    ...
    if [ -z "$OPENAI_API_KEY" ]; then
        ...
        echo "OPENAI_API_KEY found. Proceeding with runtime data loading..."
